...
function AddAddress({ close }) {
  ...
  return (
    <div>
      <section className="fixed top-0 bottom-0 left-0 right-0 p-4 bg-neutral-800 bg-opacity-60 lg:h-screen overflow-auto">
            ...
            <div className="flex gap-1">
              {data.pincode.map((_, index) => {
                  return (
                    ...
                    onKeyDown={(e) => {
                        if (e.key === "Backspace" && !data.pincode[index] && index > 0) {
                          inputRef.current[index - 1]?.focus();
                        }
                      }}
                    maxLength={1}
